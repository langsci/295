...
In some cases, semantic changes either in the verb or the noun have obscured the etymological relationship between them. For instance, it is possible that \japhug{aro}{own} is historically related to \japhug{tɤ-ro}{excess}, `surplus' (§\ref{sec:semi.transitive}), though there is no synchronic link between these two words.
...
However, goal argument saturation occurs when the base verb requires a goal argument. For instance, \japhug{ru}{look at} selects a goal or dative arguments (§\ref{sec:intr.goal}), but the incorporated verb \japhug{nɤqʰaru}{look back}, which contains the locational noun \japhug{ɯ-qʰu}{after} (with the bound form \forme{qʰa-} rather than \forme{qʰɤ-}), is strictly intransitive.
